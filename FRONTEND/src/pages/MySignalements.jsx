...
import { getSignalements, TYPES_INCIDENT } from '../api/signalementApi'
...
const MySignalements = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        const data = await getSignalements({ userId: user?.role === 'citoyen' ? user.id : undefined })
  ...
  /**
   * filteredData — Filtre les signalements selon les critères actifs
   * Appliqué en mémoire (sans appel API supplémentaire pour la démo)
   */
  ...
  return (
    <div style={{ padding: '24px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {filteredData.length === 0 ? (
        ...
        <div className="eco-card p-0" style={{ overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table className="eco-table">
              ...
              <tbody>
                ...
                {filteredData.map((s) => {
                  const typeInfo = TYPES_INCIDENT.find((t) => t.value === s.type) || {}
                  ...
                  return (
                    <tr key={s.id}>
                      ...
                      <td>
                        <div style={{ fontWeight: 600, fontSize: '0.88rem', color: 'var(--eco-text-primary)', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {s.titre}
                        </div>
                      </td>
                      {/* Badge du type */}
