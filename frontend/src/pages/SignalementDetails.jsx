...
import { getSignalementById, addCommentaire, updateStatut, getTypesIncident } from '../api/signalementApi'
...
const STATUTS = {
  ...
  valide:        { label: 'Validé', color: '#16a085', bg: 'rgba(22,160,133,0.12)' },
...
const SignalementDetails = () => {
  ...
  // Catégories officielles venant du backend pour afficher le même libellé partout.
  ...
  const typeInfo = typesIncident.find((t) => t.value === signalement.type) || {
    label: signalement.typeLabel || signalement.type || 'Type non classé',
    icon: 'bi-tags',
    color: 'var(--eco-text-secondary)',
  }
  const statutInfo = STATUTS[signalement.statut] || STATUTS.nouveau
  const canManageStatus = ['autorite', 'ministere', 'admin'].includes(user?.role)
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px' }}>
          ...
          {signalement.photoUrl && (
            <div className="eco-card p-3 mb-4">
              <div className="d-flex align-items-center justify-content-between gap-3 mb-3 flex-wrap">
                <div>
                  <h2 style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--eco-text-primary)', margin: 0 }}>
                    <i className="bi bi-image-fill me-2" style={{ color: 'var(--eco-accent)' }}></i>
                    Preuve photo du signalement
                  </h2>
                  <div style={{ color: 'var(--eco-text-secondary)', fontSize: '0.78rem', marginTop: 4 }}>
                    Image envoyee par le citoyen
                  </div>
                </div>
                <button
                  type="button"
                  className="btn btn-sm rounded-2"
                  onClick={() => setPreviewPhoto(signalement.photoUrl)}
                  style={{ border: '1px solid var(--eco-border)', color: 'var(--eco-text-primary)', background: 'var(--eco-bg-primary)' }}
                >
                  <i className="bi bi-arrows-fullscreen me-1"></i>Agrandir
                </button>
              </div>
              <button
                type="button"
                onClick={() => setPreviewPhoto(signalement.photoUrl)}
                className="eco-signalement-photo"
                aria-label="Agrandir la preuve photo"
              >
                <img src={signalement.photoUrl} alt={`Preuve du signalement ${signalement.titre}`} />
              </button>
            </div>
          )}
        ...
        <div className="col-lg-4">
          ...
          <div className="eco-card p-4 mb-4">
            ...
            <div className="d-flex flex-column gap-2">
              ...
              {signalement.adresseExacte && (
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '8px 12px', background: 'var(--eco-bg-primary)', borderRadius: 8 }}>
                  <span style={{ fontSize: '0.78rem', color: 'var(--eco-text-secondary)' }}>Adresse</span>
                  <span style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--eco-text-primary)', textAlign: 'right' }}>{signalement.adresseExacte}</span>
                </div>
              )}
            </div>
            <Link
            ...
            >
              <i className="bi bi-map me-1"></i> Voir sur la carte ECO RDC
            </Link>
          </div>

          {/* Gestion du statut reservee aux comptes institutionnels autorises. */}
          {canManageStatus && (
      ...
      <ImagePreviewModal src={previewPhoto} alt={`Preuve du signalement ${signalement.titre}`} onClose={() => setPreviewPhoto('')} />
    </div>
  )
}
