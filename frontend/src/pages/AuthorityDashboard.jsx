...
const AuthorityDashboard = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const [data, risk] = await Promise.all([getStatsAutorite(), predictRisk({ days: 7 })])
        setStats(data)
      ...
      <div className="row g-4">
        {riskPrediction && (
          <div className="col-12">
            <section className="eco-card p-4" style={{ border: '1px solid rgba(45,122,78,0.22)' }}>
              <div className="d-flex align-items-start justify-content-between flex-wrap gap-3">
                <div>
                  <h3 style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--eco-text-primary)' }}>
                    <i className="bi bi-cpu-fill me-2" style={{ color: 'var(--eco-accent)' }}></i>
                    Priorite predictive ECO IA
                  </h3>
                  <p style={{ margin: 0, color: 'var(--eco-text-secondary)' }}>{riskPrediction.recommandation}</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '1.8rem', fontWeight: 850, color: riskPrediction.score_risque >= 70 ? '#e74c3c' : 'var(--eco-accent)' }}>{riskPrediction.score_risque}%</div>
                  <div style={{ color: 'var(--eco-text-secondary)', fontSize: '0.82rem' }}>{riskPrediction.niveau_risque} - {riskPrediction.tendance}</div>
                </div>
              </div>
            </section>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '0.75rem', color: 'var(--eco-text-secondary)' }}><i className="bi bi-geo-alt me-1"></i>{u.commune}</div>
