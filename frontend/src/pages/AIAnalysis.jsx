...
// Interface connectee au backend pour soumettre une photo et recevoir une analyse predictive.
// =============================================================
import React, { useState, useRef, useEffect } from 'react'
import { analyserIncident } from '../api/aiApi'
import { getTypesIncident } from '../api/signalementApi'
...
const AIAnalysis = () => {
  ...
  // Charge les categories officielles afin que l'analyse IA utilise le meme
  // vocabulaire que les signalements et les tableaux de bord.
  useEffect(() => {
    const loadIncidentTypes = async () => {
      try {
        const categories = await getTypesIncident()
        setIncidentTypes(categories)
        setSelectedType((current) => current || categories[0]?.value || '')
      } catch (error) {
        console.error('Impossible de charger les categories IA :', error)
      }
    }
    loadIncidentTypes()
  }, [])
  ...
  /**
   * handleAnalyze — Lance l'analyse predictive via l'API IA du backend.
   */
            ...
            <div className="mt-4">
              ...
              <div className="d-flex flex-wrap gap-2">
                {incidentTypes.map((type) => {
                  const iconClass = type.icon?.startsWith('bi-') ? type.icon : `bi-${type.icon || 'tag'}`
                  return (
                    <button
                      key={type.value}
                    ...
                    >
                      <i className={`bi ${iconClass} me-2`}></i>{type.label}
                    </button>
                  )
                })}
              </div>
              ...
              <div className="animate-fade-in-up">
                ...
                <div className="mb-4">
                  ...
                  {result.providerSource && (
                    <div style={{ fontSize: '0.74rem', color: 'var(--eco-text-secondary)', marginTop: 8 }}>
                      Source IA: {result.providerSource}{result.model ? ` - ${result.model}` : ''}
                    </div>
                  )}
                </div>
